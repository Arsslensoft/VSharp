...
namespace VSC.TypeSystem.Implementation
{
	...
	/// </summary>
	public class ResolvedTypeDefinitionSpec : ITypeDefinition
	{
		...
		sealed class MemberList : IList<IMember>
		{
			...
			public MemberList(List<ITypeResolveContext> contextPerMember, List<IUnresolvedMember> unresolvedNonPartialMembers)
			{
			...
			get {
				...
				if (result == null) {
					if (this.Kind == TypeKind.Enum) {
						...
						result = SpecialTypeSpec.UnknownType;
					}
		...
		public ITypeReference ToTypeReference()
		{
			...
			if (declTypeDef != null) {
				...
				if (asm != null)
					asmRef = new AssemblyReferenceSpec(asm.AssemblyName);
				else
					asmRef = null;
		...
		{
			if ((options & GetMemberOptions.IgnoreInheritedMembers) == GetMemberOptions.IgnoreInheritedMembers) {
                return GetFilteredMethods(Base.ExtensionMethods.And(m => m.IsConstructor && !m.IsStatic, filter));
			} else {
