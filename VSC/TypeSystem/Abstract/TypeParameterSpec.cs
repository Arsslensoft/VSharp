...
{
	public abstract class TypeParameterSpec : ITypeParameter, ICompilationProvider
	{
		...
		protected TypeParameterSpec(IEntity owner, int index, string name,  DomRegion region)
		{
			if (owner == null)
		...
		protected TypeParameterSpec(ICompilation compilation, SymbolKind ownerType, int index, string name, DomRegion region)
		{
			if (compilation == null)
			...
			get {
				if (effectiveBaseClass == null) {
					...
					using (var busyLock = BusyManager.Enter(this)) {
						if (!busyLock.Success)
							return SpecialTypeSpec.UnknownType; // don't cache this error
