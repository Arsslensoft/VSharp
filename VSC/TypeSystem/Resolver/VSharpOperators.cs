...
{
	sealed class VSharpOperators
	{
		readonly ICompilation compilation;
		
		private VSharpOperators(ICompilation compilation)
		{
		...
		/// <summary>
		/// Gets the VSharpOperators instance for the specified <see cref="ICompilation"/>.
		/// This will make use of the context's cache manager (if available) to reuse the VSharpOperators instance.
		/// </summary>
		public static VSharpOperators Get(ICompilation compilation)
		{
			CacheManager cache = compilation.CacheManager;
			VSharpOperators operators = (VSharpOperators)cache.GetShared(typeof(VSharpOperators));
			if (operators == null) {
				operators = (VSharpOperators)cache.GetOrAddShared(typeof(VSharpOperators), new VSharpOperators(compilation));
			}
			return operators;
		}
		...
		void InitParameterArrays()
		{
			for (TypeCode i = TypeCode.Object; i <= TypeCode.String; i++) {
				normalParameters[i - TypeCode.Object] = new ParameterSpec(compilation.FindType(i), string.Empty);
			}
			for (TypeCode i = TypeCode.Boolean; i <= TypeCode.Decimal; i++) {
				IType type = NullableType.Create(compilation, compilation.FindType(i));
                nullableParameters[i - TypeCode.Boolean] = new ParameterSpec(type, string.Empty);
			}
		}
		...
		internal class OperatorMethod : IParameterizedMember
		{
			...
			public virtual OperatorMethod Lift(VSharpOperators operators)
			{
				return null;
			}
		...
		sealed class LambdaUnaryOperatorMethod<T> : UnaryOperatorMethod
		{
			...
			public LambdaUnaryOperatorMethod(VSharpOperators operators, Func<T, T> func)
			...
			public override object Invoke(ResolveContext resolver, object input)
			{
				if (input == null)
					return null;
				return func((T)resolver.VSharpPrimitiveCast(Type.GetTypeCode(typeof(T)), input));
			}
			
			public override OperatorMethod Lift(VSharpOperators operators)
			{
		...
		sealed class LiftedUnaryOperatorMethod : UnaryOperatorMethod, OverloadResolution.ILiftedOperator
		{
			...
			public LiftedUnaryOperatorMethod(VSharpOperators operators, UnaryOperatorMethod baseMethod) : base(operators.compilation)
			{
		...
		// V# 4.0 spec: §7.7.1 Unary plus operator
		...
		// V# 4.0 spec: §7.7.2 Unary minus operator
		...
		// V# 4.0 spec: §7.7.3 Logical negation operator
		...
		// V# 4.0 spec: §7.7.4 Bitwise complement operator
		...
		sealed class LambdaBinaryOperatorMethod<T1, T2> : BinaryOperatorMethod
		{
			...
			public LambdaBinaryOperatorMethod(VSharpOperators operators, Func<T1, T2, T1> func)
				: this(operators, func, func)
			{
			}
			
			public LambdaBinaryOperatorMethod(VSharpOperators operators, Func<T1, T2, T1> checkedFunc, Func<T1, T2, T1> uncheckedFunc)
				: base(operators.compilation)
			{
			...
			public override object Invoke(ResolveContext resolver, object lhs, object rhs)
			{
				...
				return func((T1)resolver.VSharpPrimitiveCast(Type.GetTypeCode(typeof(T1)), lhs),
				            (T2)resolver.VSharpPrimitiveCast(Type.GetTypeCode(typeof(T2)), rhs));
			}
		...
		sealed class LiftedBinaryOperatorMethod : BinaryOperatorMethod, OverloadResolution.ILiftedOperator
		{
			...
			public LiftedBinaryOperatorMethod(VSharpOperators operators, BinaryOperatorMethod baseMethod)
				: base(operators.compilation)
			{
				this.baseMethod = baseMethod;
		...
		// V# 4.0 spec: §7.8.1 Multiplication operator
		...
		// V# 4.0 spec: §7.8.2 Division operator
		...
		// V# 4.0 spec: §7.8.3 Remainder operator
		...
		// V# 4.0 spec: §7.8.3 Addition operator
		...
		sealed class StringConcatenation : BinaryOperatorMethod
		{
			...
			public StringConcatenation(VSharpOperators operators, TypeCode p1, TypeCode p2)
				: base(operators.compilation)
			{
		...
		// V# 4.0 spec: §7.8.4 Subtraction operator
		...
		// V# 4.0 spec: §7.8.5 Shift operators
		...
		sealed class EqualityOperatorMethod : BinaryOperatorMethod
		{
			...
			public EqualityOperatorMethod(VSharpOperators operators, TypeCode type, bool negate)
				: base(operators.compilation)
			{
				this.Negate = negate;
				this.Type = type;
			...
			public override object Invoke(ResolveContext resolver, object lhs, object rhs)
			{
				...
				lhs = resolver.VSharpPrimitiveCast(Type, lhs);
				rhs = resolver.VSharpPrimitiveCast(Type, rhs);
				bool equal;
		...
		sealed class LiftedEqualityOperatorMethod : BinaryOperatorMethod, OverloadResolution.ILiftedOperator
		{
			...
			public LiftedEqualityOperatorMethod(VSharpOperators operators, EqualityOperatorMethod baseMethod)
				: base(operators.compilation)
			{
				this.baseMethod = baseMethod;
		...
		// V# 4.0 spec: §7.10 Relational and type-testing operators
		...
		sealed class RelationalOperatorMethod<T1, T2> : BinaryOperatorMethod
		{
			...
			public RelationalOperatorMethod(VSharpOperators operators, Func<T1, T2, bool> func)
				: base(operators.compilation)
			{
				this.ReturnType = operators.compilation.FindType(KnownTypeCode.Boolean);
