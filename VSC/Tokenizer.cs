...
namespace VSC
{
    ...
    public class Tokenizer : yyParser.yyInput
    {
        ...
        public bool IsEscapedIdentifier(TypeNameExpression name)
        {
        ...
        int GetKeyword(char[] id, int id_len)
        {
            ...
            switch (res)
            {
                ...
                case Token.DEFAULT:
                    ...
                    break;
                case Token.WHEN:
                    if (current_token != Token.EXCEPT && !parsing_catch_when)
                        res = -1;
                    break;
                case Token.WHERE:
                    ...
                    if (!query_parsing)
                    {
                        ...
                        switch (xtoken())
                        {
                            ...
                            case Token.ULONG:
                                ...
                                query_parsing = true;
                                if (context.Settings.Version <= LanguageVersion.V_1)
                                    Report.FeatureIsNotAvailable(context, Location, "query expressions");
                                break;
                            case Token.VOID:
                                //          Expression.Error_VoidInvalidInTheContext(Location, Report);//TODO:Fix this
                                break;
                            default:
                                PopPosition();
                ...
                case Token.PARTIAL:
                    ...
                    if (ok)
                    {
                        ...
                        else if (context.Settings.Version == LanguageVersion.V_1)
        ...
        ILiteralConstant integer_type_suffix(ulong ul, int c, Location loc)
        {
            ...
            {
                return new ULongLiteral( ul, loc);
            }
            ...
            if (is_unsigned)
            {
                ...
                if ((ul & 0xffffffff00000000) == 0)
                    return new UIntLiteral( (uint)ul, loc);
                else
                    return new ULongLiteral( ul, loc);
            }
            else if (is_long)
            {
                ...
                if ((ul & 0x8000000000000000) != 0)
                    ...
                    return new LongLiteral( (long)ul, loc);
            }
            else
            {
                ...
                if ((ul & 0xffffffff00000000) == 0)
                {
                    ...
                    if ((ui & 0x80000000) != 0)
                        return new UIntLiteral( ui, loc);
                    else
                        return new IntLiteral( (int)ui, loc);
                }
                else
                {
                    if ((ul & 0x8000000000000000) != 0)
        ...
        ILiteralConstant adjust_int(int c, Location loc)
        {
            ...
            {
                Error_NumericConstantTooLong();
                return new IntLiteral( 0, loc);
            }
            catch (FormatException)
            {
                Report.Error(12, Location, "Invalid number");
                return new IntLiteral( 0, loc);
            }
        }
        ...
        ILiteralConstant adjust_real(TypeCode t, Location loc)
        {
            ...
            switch (t)
            {
                ...
                case TypeCode.Single:
                    try
                    {
                        return new FloatLiteral( float.Parse(s, styles, csharp_format_info), loc);
                    }
                    catch (OverflowException)
                    {
                        Report.Error(13, Location, error_details, "float");
                        return new FloatLiteral( 0, loc);
                    }
                ...
                default:
                    try
                    {
                        return new DoubleLiteral( double.Parse(s, styles, csharp_format_info), loc);
                    }
                    catch (OverflowException)
                    {
                        Report.Error(13, loc, error_details, "double");
                        return new DoubleLiteral( 0, loc);
                    }
            }
        ...
        int escape(int c, out int surrogate)
        {
            ...
            switch (d)
            {
                ...
                default:
                    surrogate = 0;
                    Report.Error(14, Location, "Unrecognized escape sequence `\\{0}'", ((char)d).ToString());
                    return d;
            }
        ...
        int EscapeUnicode(int ch, out int surrogate)
        {
            ...
            if (error)
                Report.Error(14, Location, "Unrecognized escape sequence");
        ...
        bool IsInAsm = false;
        bool IsLastTokenAsm = false;
        public static string AsmCode = null;

        public int token()
        {
            current_token = xtoken();
            //if (current_token == Token.ASM)
            //    IsLastTokenAsm = true;
            //else if (current_token == Token.OPEN_BRACE && IsLastTokenAsm)
            //{
            //    IsInAsm = true;
            //    AsmCode = "";
            //}
            //else if (current_token == Token.CLOSE_BRACE && IsInAsm)
            //{
            IsInAsm = false;
            //if (AsmCode != null)
            //{
            //    AsmCodeDef acd = new AsmCodeDef();
            //    acd.AsmCode = AsmCode;
            //    acd.File = Location.File;
        ...
        bool PreProcessLine()
        {
            ...
            if (c != '\n' && c != '/' && c != '"' && c != UnicodeLS && c != UnicodePS)
            {
                ...
                Report.Error(15, loc, "Filename, single-line comment or end-of-line expected");
                return true;
            }
        ...
        void PreProcessDefinition(bool is_define, string ident, bool caller_is_taking)
        {
            ...
            {
                Report.Error(16, Location, "Missing identifier to pre-processor directive");
                return;
            }
            ...
            if (!is_identifier_start_character(ident[0]))
                Report.Error(16, Location, "Identifier expected: {0}", ident);
            ...
            if (is_define)
            {
                ...
                // #define ident
                //
                if (context.Settings.Symbols.Contains(ident))
                    return;
                ...
                else
                {
                    ...
                    while (c < MaxIdentifierLength)
                    {
                        ...
                        if (c >= '0' && c <= '9')
                        {
                            ...
                            {
                                // Recognize CSXXXX as V# XXXX warning
                                number = 0;
                                int pow = 1000;
                                for (int i = 0; i < 4; ++i)
                                {
                ...
                if (length == pragma_warning_disable.Length)
                {
                    ...
                    if (disable || IsTokenIdentifierEqual(pragma_warning_restore))
                    {
                        ...
                        if (c == '\n' || c == '/' || c == UnicodeLS || c == UnicodePS)
                        {
                            ...
                            if (disable)
                            {
                                Report.RegisterWarningRegion(loc).WarningDisable(loc.Line);
                            }
                            else
                            {
                                Report.RegisterWarningRegion(loc).WarningEnable(loc.Line);
                            }
                        }
                        else
                        {
        ...
        {
            Report.Error(17, Location, "Integral constant is too large");
        }

        void Error_InvalidDirective()
        {
            Report.Error(18, Location, "Invalid preprocessor directive");
        }
        ...
        {
            Report.Error(20, Location,
                "Cannot define or undefine preprocessor symbols after first token in file");
        }

        void Eror_WrongPreprocessorLocation()
        {
            Report.Error(21, Location,
                "Preprocessor directives must appear as the first non-whitespace character on a line");
        }

        void Error_EndLineExpected()
        {
            Report.Error(22, Location, "Single-line comment or end-of-line expected");
        }
        ...
        void WarningMisplacedComment(Location loc)
        {
            if (doc_state != XmlCommentState.Error)
            {
                ...
                Report.Warning(23, 2, loc, "XML comment is not placed on a valid language element");
            }
        }
        ...
        bool ParsePreprocessingDirective(bool caller_is_taking)
        {
            ...
            switch (directive)
            {
                ...
                case PreprocessorDirective.Endregion:
                    ...
                    if ((pop & REGION) == 0)
                        Report.Error(24, Location, "Expected `#endif' directive");
                ...
                case PreprocessorDirective.Endif:
                    ...
                    else
                    {
                        ...
                        if ((pop & REGION) != 0)
                            Report.Error(25, Location, "#endregion directive expected");
                ...
                case PreprocessorDirective.Invalid:
                    Report.Error(26, Location, "Wrong preprocessor directive");
                    return true;
            }
            ...
            switch (directive)
            {
                case PreprocessorDirective.Error:
                    Report.Error(27, Location, "#error: '{0}'", arg);
                    return true;
                ...
                case PreprocessorDirective.Line:
                    Location loc = Location;
                    if (!PreProcessLine())
                        Report.Error(28, loc, "The line number specified for #line directive is missing or invalid");
        ...
        int consume_string(bool quoted)
        {
            ...
            while (true)
            {
                ...
                if (c == '"')
                {
                    ...
                    ILiteralConstant res = new StringLiteral( CreateStringFromBuilder(pos), start_location);
                    val = res;
                ...
                if (c == '\n' || c == UnicodeLS || c == UnicodePS)
                {
                    if (!quoted)
                    {
                        Report.Error(29, Location, "Newline in constant");
                        ...
                        val = new StringLiteral( new string(value_builder, 0, pos), start_location);
                ...
                {
                    Report.Error(30, Location, "Unterminated string literal");
                    return Token.EOF;
                }
                else
                {
        ...
        int consume_identifier(int c, bool quoted)
        {
            ...
            if (c == '\\')
            {
                ...
                {
                    Report.Error(31, Location, "Unexpected character `\\{0}'", c.ToString("x4"));
                    return Token.ERROR;
                }
            }
            ...
            {
                Report.Error(32, Location, "Identifier too long (limit is 512 chars)");
                --pos;
                col += pos;
            }
            ...
            if (id_builder[0] >= '_' && !quoted)
            {
                ...
                if (keyword != -1)
                {
                    val = ltb.Create( null, current_source, ref_line, column);
                    return keyword;
                }
            }
        ...
        public int xtoken()
        {
            ...
            while ((c = get_char()) != -1)
            {
                    ...
                    */
                    ...
                    case '/':
                        ...
                        if (d == '/')
                        {
                            if (parsing_string_interpolation > 0)
                            {
                                Report.Error(33, Location, "A single-line comment may not be used in an interpolated string");
                                goto case '}';
                            }
                        ...
                        else if (d == '*')
                        {
                            ...
                            if (!comments_seen)
                                Report.Error(34, Location, "End-of-file found, '*/' expected");
                    ...
                    case '"':
                        if (parsing_string_interpolation > 0 && !ScanClosingInterpolationBrace())
                        {
                            ...
                            Report.Error(35, Location, "Missing close delimiter `}' for interpolated expression");
                            val = new StringLiteral( "", Location);
                    ...
                    case '@':
                        ...
                        Report.Error(36, Location, "Keyword, identifier, or string expected after verbatim specifier: @");
                        return Token.ERROR;
                    ...
                    case '$':
                        ...
                        if (d == '>' && IsInAsm)
                        {
                            ...
                            if (doc_processing && peek_char() == '<')
                            {
                                ...
                                Report.Warning(3645, 2, Location, "Asm code $><$");
                            }


                            while ((d = get_char()) != -1 && IsInAsm)
                            {
                                if (d == '<' && peek_char() == '$')
                                {
                                    get_char();
                            ...
                            if (d == -1)
                                Report.Error(37, Location, "End-of-file found, '<$' expected");
                ...
                Report.Error(31, Location, "Unexpected character `{0}'", ((char)c).ToString());
            }
        ...
        int TokenizeBackslash()
        {
            ...
            if (c == '\'')
            {
                val = new CharLiteral( (char)c, start_location);
                Report.Error(37, start_location, "Empty character literal");
                return Token.LITERAL;
            }

            if (c == '\n' || c == UnicodeLS || c == UnicodePS)
            {
                Report.Error(29, start_location, "Newline in constant");
                return Token.ERROR;
            }
            ...
            ILiteralConstant res = new CharLiteral( (char)c, start_location);
            val = res;
            c = get_char();
            ...
            while (true)
            {
                ...
                switch (ch)
                {
                    case '"':
                        ...
                        val = new StringLiteral( CreateStringFromBuilder(pos), start_location);
                        return Token.INTERPOLATED_STRING_END;
                    case '{':
                        if (peek_char() == '{')
                        {
                    ...
                    case '\\':
                        ...
                        if (ch == '{' || ch == '}')
                        {
                            Report.Error(39, Location, "A `{0}' character may only be escaped by doubling `{0}{0}' in an interpolated string", ((char)ch).ToString());
                        }
                ...
                switch (ch)
                {
                    ...
                    case '}':
                        if (braces == 0)
                        {
                            ...
                            if (pos == 0)
                            {
                                Report.Error(40, Location, "Empty interpolated expression format specifier");
                            }
                            else if (Array.IndexOf(simple_whitespaces, value_builder[pos - 1]) >= 0)
                            {
                                Report.Error(41, Location, "A interpolated expression format specifier may not contain trailing whitespace");
                            }
