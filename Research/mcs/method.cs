		...
		public override string GetSignatureForError ()
		{
			...
			if (IsConstructor) {
				...
				if (split == 3) {
					...
					if (pc > 0 && postfix == "get") {
						name = "self" + parameters.GetSignatureForError ("[", "]", pc);
					} else if (pc > 1 && postfix == "set") {
						name = "self" + parameters.GetSignatureForError ("[", "]", pc - 1);
					}
				}
...
//				return ginstance;
	...
	public abstract class MethodOrOperator : MethodCore, IMethodData, IMethodDefinition
	{
		...
		static readonly string[] attribute_targets = new string [] { "method", "ret" };
		...
		public override void Emit ()
		{
			...
			if (block != null) {
				...
				if (block.Resolve (bc, this)) {
                    ...
                    //if (Parent.SynchronizedFields != null)
                    //{
                    //    foreach (FieldSpec fs in Parent.SynchronizedFields)
                    //    {
                    //        if (((ModFlags & Modifiers.STATIC) == (fs.Modifiers & Modifiers.STATIC)))
                    //        {
                    //            //ec.Emit(OpCodes.Ldc_I4_1);
                    //            //ec.Emit(OpCodes.Box, ec.BuiltinTypes.Int);
                    //            ec.Emit(OpCodes.Ldnull);
                    //            ec.Emit(OpCodes.Stfld, fs);
                    //        }
                    //    }
                    //}
					block.Emit (ec);
	...
	public class MethodData
	{
        ...
        void DoEmitSync(TypeDefinition parent, ToplevelBlock block)
        {
            if (block != null)
            {
                Modifiers field_mods = Modifiers.PRIVATE;
                BlockContext bc = new BlockContext(method, block, method.ReturnType);
                string field_name = CompilerGeneratedContainer.MakeName(parent.Definition.Name, "f", "synchronized", 0);
                if ((modifiers & Modifiers.STATIC) == Modifiers.STATIC)
                    field_mods |= Modifiers.STATIC;
               
                Field sync_field = new Field(parent, new TypeExpression(parent.Compiler.BuiltinTypes.Object, parent.Module.Location),field_mods | Modifiers.COMPILER_GENERATED, new MemberName(field_name), null);
                if (sync_field.Define())
                {
                    parent.AddField(sync_field);
                    sync_field.Emit();

                    FieldExpr sync_fexpr = new FieldExpr(sync_field.Spec, parent.Module.Location);


                    // Copy all statements from block to sync_method_body and clear
                    ExplicitBlock sync_method_body = new ExplicitBlock(block, block.StartLocation, block.EndLocation);
                    foreach (Statement st in block.Statements)
                        sync_method_body.AddStatement(st);
                    block.Statements.Clear();

                    // Create a Synchronized Statement
                    SynchronizedMethod sm = new SynchronizedMethod(sync_fexpr, sync_method_body, method, parent.Module.Location);
                    block.AddStatement(sm);


                    if (block.Resolve(bc, method) && sm.Resolve(bc))
                    {
                ...
                else parent.Compiler.Report.Error(3660, "Failed to declare a sync method");
            }
            else if ((member.ModFlags & Modifiers.ABSTRACT) == Modifiers.ABSTRACT)
              parent.Compiler.  Report.Error(3661,  method.Location , "An abstract sub method cannot use sync modifier");
        }
		//
		// Emits the code
		// 
		public void Emit (TypeDefinition parent)
		{
            ...
            // Emit Sync Method
            if ((modifiers & Modifiers.SYNC) == Modifiers.SYNC)
                DoEmitSync(parent,block);
            // Emit Normal Method
            else if (block != null) {
				BlockContext bc = new BlockContext (method, block, method.ReturnType);
	...
	public class Operator : MethodOrOperator {
		...
		static Operator ()
		{
            ...
            names[(int)OpType.BitwiseNand] = new string[] { "!&", "op_Nand" };
            names[(int)OpType.BitwiseNor] = new string[] { "!|", "op_Nor" };
            names[(int)OpType.BitwiseXnor] = new string[] { "!^", "op_Xnor" };
		}
