...
namespace Mono.CSharp
{
	...
	public class Binary : Expression, IDynamicBinder
	{
		...
		public enum Operator {
            ...
            BitwiseXnor = 19 | BitwiseMask,
            BitwiseNand = 20 | BitwiseMask,
            BitwiseNor = 21  | BitwiseMask,
			//
			// Operator masks
			//
		...
		string OperName (Operator oper)
		{
            ...
            case Operator.BitwiseXnor:
                s = "!^";
                break;
            case Operator.BitwiseNor:
                s = "!|";
                break;
		...
		static CSharp.Operator.OpType ConvertBinaryToUserOperator (Operator op)
		{
            ...
            case Operator.BitwiseNand:
                return CSharp.Operator.OpType.BitwiseNand;
            case Operator.BitwiseNor:
                return CSharp.Operator.OpType.BitwiseNor;
            case Operator.BitwiseXnor:
                return CSharp.Operator.OpType.BitwiseXnor;
			case Operator.RightShift:
		...
		public static void EmitOperatorOpcode (EmitContext ec, Operator oper, TypeSpec l, Expression right)
		{
            ...
            case Operator.Power:
                ...
                ec.Emit(OpCodes.Call, ec.Module.PredefinedMembers.Power.Resolve(ec.Module.Location));               
                opcode = EmitConv(ec, l);
                break;
			case Operator.Modulus:
				if (IsUnsigned (l))
        ...
        static OpCode EmitConv(EmitContext ec, TypeSpec l)
        {
            // Corrected Values
            if (l.BuiltinType == BuiltinTypeSpec.Type.Int)
            {
                ec.Emit(OpCodes.Conv_I4);
                goto correction;
     
            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.UInt)
            {
                ec.Emit(OpCodes.Conv_U4);
                goto correction;

            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.Short)
            {
                ec.Emit(OpCodes.Conv_I2);
                goto correction;

            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.UShort)
            {
                ec.Emit(OpCodes.Conv_U2);
                goto correction;

            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.SByte)
            {
                ec.Emit(OpCodes.Conv_I1);
                goto correction;

            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.Byte)
            {
            ...
            else if (l.BuiltinType == BuiltinTypeSpec.Type.Long)
            {
                ec.Emit(OpCodes.Conv_I8);
                goto correction;

            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.ULong)
            {
                ec.Emit(OpCodes.Conv_U8);
                goto correction;

            }
            else if (l.BuiltinType == BuiltinTypeSpec.Type.Float)
		...
		public SLE.Expression MakeExpression (BuilderContext ctx, Expression left, Expression right)
		{
            ...
            case Operator.BitwiseXnor:
                return  SLE.Expression.Not(SLE.Expression.ExclusiveOr(le, re));
            case Operator.BitwiseNor:
                return SLE.Expression.Not(SLE.Expression.Or(le, re));
            case Operator.BitwiseNand:
                return SLE.Expression.Not(SLE.Expression.And(le, re));
			case Operator.Multiply:
	...
	public class LocalVariableReference : VariableReference
	{
      ...
      public override void EmitAssignFromStack(EmitContext ec)
        {
            ec.Emit(OpCodes.Stloc,local_info.builder);
        }
	...
	public class ParameterReference : VariableReference
	{
        ...
        public override void EmitAssignFromStack(EmitContext ec)
        {
            ec.Emit(OpCodes.Starg, pi.Parameter.Index);
        }
