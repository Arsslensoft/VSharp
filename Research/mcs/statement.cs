...
//					rc.Report.ExtraInformation (loc, "(Ambiguous implicit user defined conversion in previous ");
   ...
   public class Safe : Statement
    ...
    public class AsmInline : Statement
    {
        public Block Block;

        public static EAX EAX_REG;
        public static EBX EBX_REG;
        public static ECX ECX_REG;
        public static EDX EDX_REG;

        public static ESI ESI_REG;
        public static EDI EDI_REG;



        public static RBX RBX_REG;
        public static RCX RCX_REG;
        public static RDX RDX_REG;
        public static RAX RAX_REG;

        public static RSI RSI_REG;
        public static RDI RDI_REG;

        public static GBL GBL_REG;
        public static MEM MEM_REG;

        public static List<Register> DefinedRegisters { get; set; }


        List<AsmCodeDef> CodeDef { get; set; }
        InlineAsmDeclaration CurrentDeclaration { get; set; }
        public static TypeBuilder AssemblerClassBuilder = null;
        public static PredefinedType AsmPinvokeClass = null;
        public static NativeCodeDeclaration NCD { get; set; }



        //AsmDeclaration asmdecl;
        public AsmInline(Block b, Location loc)
        {
           
            
            Block = b;
            Block.Unsafe = false;
            CodeDef = new List<AsmCodeDef>();
            CurrentDeclaration = new InlineAsmDeclaration(b.StartLocation);
            if (EAX_REG == null)
            {
                DefinedRegisters = new List<Register>();
                EAX_REG = new EAX();
                EBX_REG = new EBX();
                ECX_REG = new ECX();
                EDX_REG = new EDX();
                ...
                DefinedRegisters.Add(EAX_REG);
                DefinedRegisters.Add(EBX_REG);
                DefinedRegisters.Add(ECX_REG);
                DefinedRegisters.Add(EDX_REG);
                DefinedRegisters.Add(EDI_REG);
                DefinedRegisters.Add(ESI_REG);


                DefinedRegisters.Add(RAX_REG);
                DefinedRegisters.Add(RBX_REG);
                DefinedRegisters.Add(RCX_REG);
                DefinedRegisters.Add(RDX_REG);
                DefinedRegisters.Add(RSI_REG);
                DefinedRegisters.Add(RDI_REG);

                DefinedRegisters.Add(GBL_REG);
                DefinedRegisters.Add(MEM_REG);
            }
            if (NCD == null)
            {
                NCD = new NativeCodeDeclaration();
                NCD.Includes.Add("<windows.h>");
                NCD.Includes.Add("<stdio.h>");
            }
            // PushArgs = new List<Expression>();
            //Args = new List<Expression>();
            // InputOperands = new List<string>();
            // OutputOperands = new List<string>();
            // ReturnVariable = null;
            // AsmCode = null;
            // ReturnVarExpr = null;
            // ReturnType = "void";
        ...
        bool IsInThisAsmStatement(AsmCodeDef acd)
        {
            if (Block.EndLocation.File != acd.File)
                return false;

            if (Block.EndLocation.Row < acd.Line)
                return false;

            if (Block.EndLocation.Row == acd.Line && Block.EndLocation.Column < acd.Col)
                return false;

            return true;
        }

        public void ParseAsmBlock(Block b, ResolveContext rc)
        {

            // Get asm code

            foreach (AsmCodeDef ad in Tokenizer.AsmCodes)
                if (IsInThisAsmStatement(ad))
                    CodeDef.Add(ad);
            // Remove from tokenizer list
            foreach (AsmCodeDef acd in CodeDef)
                Tokenizer.AsmCodes.Remove(acd);


            // Get args
            foreach (Statement st in b.Statements)
            {
                if (st is StatementExpression && ((StatementExpression)st).Expr is SimpleAssign)
                {
                    AsmCodeDef acdef = new AsmCodeDef();

                    acdef.File = st.loc.File;
                    acdef.Col = st.loc.Column;
                    acdef.Line = st.loc.Row;

                    acdef.AssignInstruction = ((SimpleAssign)((StatementExpression)st).Expr);

                    CodeDef.Add(acdef);
                    //SimpleName src = ((SimpleAssign)((StatementExpression)st).Expr).Source as SimpleName;
                    //SimpleName trg = ((SimpleAssign)((StatementExpression)st).Expr).Target as SimpleName;

                    //if (trg.Name == "PUSHARG")
                    //{
                    //    PushArgs.Add(((SimpleAssign)((StatementExpression)st).Expr).Source);

                    //}

                    //else if (trg.Name == "OUTOP")
                    //{
                    //    StringLiteral sval = ((SimpleAssign)((StatementExpression)st).Expr).Source as StringLiteral;
                    //    if (sval != null)
                    //    OutputOperands.Add(sval.Value);
                ...
                else rc.Report.Error(3667, st.loc, "Asm statement supports only predefined simple assigns expressions.");


            }
            CodeDef.Sort();
        }
        void AddParameter(AsmParameter ni)
        {
            int i = 0;
            for (i = 0; i < CurrentDeclaration.Parameters.Count; i++)
                if (CurrentDeclaration.Parameters[i].ParameterName == ni.ParameterName)
                {
                    if (CurrentDeclaration.Parameters[i].IsOutParameter)
                        return;
                    else if (ni.IsOutParameter)
                        CurrentDeclaration.Parameters[i] = ni;
                    return;
                }

            CurrentDeclaration.Parameters.Add(ni);

        }


        bool AddAssignRegister(ResolveContext rc, SimpleName src, SimpleName trg, AsmCodeDef acd, ref Expression tmp)
        {
            tmp = null;
            foreach (Register reg in DefinedRegisters)
            {
                // ASSIGN REGISTERS
                if (trg != null && trg.Name == reg.Name && !CurrentDeclaration.InputRegisters.Contains(reg))
                {
                    // CHECK64BITS
                    if (reg.Name != "GBL" && reg.Name != "MEM")
                    {
                        if ((rc.Module.Compiler.Settings.Platform == Platform.AnyCPU32Preferred || rc.Module.Compiler.Settings.Platform == Platform.X86 || rc.Module.Compiler.Settings.Platform == Platform.AnyCPU) && reg.Is64BitRegister)
                            AssemblerErrorManager.AsmGenErrorRegister(rc.Report, ". Register " + reg.Name);
                        else if ((rc.Module.Compiler.Settings.Platform == Platform.IA64 || rc.Module.Compiler.Settings.Platform == Platform.X64) && !reg.Is64BitRegister)
                            AssemblerErrorManager.AsmGenErrorRegister(rc.Report, ". Register " + reg.Name);
                    }
                    CurrentDeclaration.InputRegisters.Add(reg);



                    tmp = acd.AssignInstruction.Source;

                    // Check if is constant or var
                    if (ResolveArgumentExpression(rc, ref  tmp))
                    {
                        if (acd.AssignInstruction.Source is Constant)
                        {

                            string vp = "vp_" + CurrentDeclaration.Parameters.Count.ToString();
                            AsmParameter ap = new AsmParameter(vp, acd.AssignInstruction.Source, false, rc.Module.Builder.universe);
                            CurrentDeclaration.InputOperands.Add(new Operand(OperandType.REGISTER_ASSIGN, ap, reg));
                            AddParameter(ap);
                        ...
                        else
                        {
                            ...
                            //if (tp != tmp.Type)
                            //    AssemblerErrorManager.AsmGenErrorRegisterTypeMismatch(rc.Report, ". Register "+reg.Name + " is typeof "+reg.NType.ToString());



                            AsmParameter ap = new AsmParameter(src.Name, tmp, false, rc.Module.Builder.universe);
                            CurrentDeclaration.InputOperands.Add(new Operand(OperandType.REGISTER_ASSIGN, ap, reg));
                            AddParameter(ap);
                        }
                        return true;
        ...
        bool AddLoadRegister(ResolveContext rc, SimpleName src, SimpleName trg, AsmCodeDef acd, ref Expression tmp)
        {
            tmp = null;
            foreach (Register reg in DefinedRegisters)
            {
                if (src != null && src.Name == reg.Name && !CurrentDeclaration.OutputRegisters.Contains(reg))
                {
                    // CHECK64BITS
                    if (reg.Name != "GBL" && reg.Name != "MEM")
                    {
                        if ((rc.Module.Compiler.Settings.Platform == Platform.AnyCPU32Preferred || rc.Module.Compiler.Settings.Platform == Platform.X86) && reg.Is64BitRegister)

                            AssemblerErrorManager.AsmGenErrorRegister(rc.Report, ". Register " + reg.Name);
                    }

                    CurrentDeclaration.OutputRegisters.Add(reg);
                    // Check if is constant or var
                    tmp = acd.AssignInstruction.Target;
                    if (ResolveArgumentExpression(rc, ref  tmp))
                    {
                        if (acd.AssignInstruction.Source is Constant)
                            AssemblerErrorManager.AsmGenErrorParam(rc.Report);
                        ...
                        else
                        {
                            ...
                            AsmParameter ap = new AsmParameter(trg.Name, tmp, true, rc.Module.Builder.universe);
                            CurrentDeclaration.OutputOperands.Add(new Operand(OperandType.REGISTER_LOAD, ap, reg));
                            AddParameter(ap);
                            CurrentDeclaration.OutputVars.Add(trg.Name);
                        }
                        return true;
                    }
        ...
        bool AddOverride(ResolveContext rc, StringLiteral src, SimpleName trg)
        {
            if (trg != null && trg.Name == "OVROP" && src != null)
            {
                foreach (string svar in src.Value.Split('|'))
                {
                    string[] vars = svar.Split(',');
                    if (CurrentDeclaration.OverrideVars.ContainsKey(vars[0]))
                        CurrentDeclaration.OverrideVars[vars[0]] = vars[1];
                    else
                        CurrentDeclaration.OverrideVars.Add(vars[0], vars[1]);
        ...
        bool AddInclude(ResolveContext rc, StringLiteral src, SimpleName trg)
        {
            if (trg != null && trg.Name == "INCL" && src != null)
            {
                if (!NCD.Includes.Contains(src.Value))
                    NCD.Includes.Add(src.Value);
        ...
        public void ResolveDefinitions(ResolveContext rc)
        {
            bool ret = false;
            Expression tmp = null;
            foreach (AsmCodeDef acd in CodeDef)
            {
                ret = false;
                tmp = null;
                if (acd.AssignInstruction != null)
                {
                    //asign
                    // check registers
                    SimpleName src = acd.AssignInstruction.Source as SimpleName;
                    SimpleName trg = acd.AssignInstruction.Target as SimpleName;

                    // ASSIGN REGISTERS
                    ret |= AddAssignRegister(rc, src, trg, acd, ref tmp);
                    // LOAD REGISTERS
                    if (!ret)
                        ret |= AddLoadRegister(rc, src, trg, acd, ref tmp);
                    // OVERIDE
                    if (!ret)
                        ret |= AddOverride(rc, acd.AssignInstruction.Source as StringLiteral, trg);
                    // INCLUDE
                    if (!ret)
                        ret |= AddInclude(rc, acd.AssignInstruction.Source as StringLiteral, trg);
                    if (!ret)
                        rc.Report.Error(3667, acd.AssignInstruction.Location, "Asm statement does not support this expression");

                    LocalVariableReference lvar = tmp as LocalVariableReference;
                    if (lvar != null)
                        lvar.local_info.SetIsUsed();

                }
                else if (acd.AsmCode != null)
                    CurrentDeclaration.AsmCode = acd.AsmCode;

            }


        }
        bool ResolveArgumentExpression(ResolveContext rc, ref Expression expr)
        {
            ...
            if (sn != null)
            {
                ...
                expr = sn.LookupNameExpression(rc, Mono.CSharp.Expression.MemberLookupRestrictions.IgnoreAmbiguity | Mono.CSharp.Expression.MemberLookupRestrictions.NameOfExcluded);
                if (sn.HasTypeArguments && expr is MethodGroupExpr)
                    rc.Report.Error(8084, sn.Location, "An argument to asm operator cannot be method group with type arguments");


                return true;
            }
            // literal constants
            if (intc != null || boolc != null || uintc != null || shortc != null | ushortc != null || longc != null || ulongc != null || bytec != null || strc != null || dblc != null || floatc != null)
            {

                //Args.Add(expr);
                //variables.Add("vp_" + variables.Count.ToString());
                return true;
            }
        ...
        public override bool Resolve(BlockContext ec)
        {
            if (ec.Module.Compiler.Settings.IntelAsm)
            {
                ParseAsmBlock(Block, ec);
                ResolveDefinitions(ec);
                //foreach (Expression expr in PushArgs)
                //    if (!ResolveArgumentExpression(ec, expr))
                //        return false;

                //if (ReturnVarExpr is SimpleName  )
                //{
                //    SimpleName sn = ReturnVarExpr as SimpleName;
                //     ReturnVarExpr= sn.LookupNameExpression(ec, Mono.CSharp.Expression.MemberLookupRestrictions.IgnoreAmbiguity | Mono.CSharp.Expression.MemberLookupRestrictions.NameOfExcluded);
                //     if (sn.HasTypeArguments && ReturnVarExpr is MethodGroupExpr)
                //          ec.Report.Error(8084, sn.Location, "An argument to asm operator cannot be method group with type arguments");


                NCD.AssemblyDeclarations.Add(CurrentDeclaration);
                NCD.EmitedDeclarations.Add(CurrentDeclaration.MethodName);
                //}
                //else ec.Report.Error(5555, "Asm return variable must be declared in the current context");
            ...
            {
                ec.Report.Error(3659, "Intelligent Assembly is not enabled");
                return false;
            }
        }
        ...
        protected override void DoEmit(EmitContext ec)
        {
            ...
            if (AssemblerClassBuilder == null)
                AssemblerClassBuilder = ec.Module.Builder.DefineType("AlLanguageHandling.AlAsmPInvoke", IKVM.Reflection.TypeAttributes.Class | IKVM.Reflection.TypeAttributes.Public);
            MethodBuilder mb = AssemblerGenerator.GenerateMethod(ec, CurrentDeclaration, AssemblerClassBuilder, ec.Module.Builder.universe);
            AssemblerGenerator.EmitStackCall(ec, mb, CurrentDeclaration, ec.LocalVariablesIdx);
            NCD.EmitedDeclarations.Remove(CurrentDeclaration.MethodName);
    ...
    public class SynchronizedMethod : TryFinallyBlock
    {
        Expression expr;
        TemporaryVariableReference expr_copy;
        TemporaryVariableReference lock_taken;
        IMethodData methodData;
        public SynchronizedMethod(Expression expr, Statement stmt, IMethodData md, Location loc)
            : base(stmt, md, loc)
        {
            this.expr = expr;
            methodData = md;
        ...
        {
            Label l_initialized = ec.DefineLabel();
            FieldExpr fexpr = expr as FieldExpr;

            if (fexpr.Spec != null)
            {
                ec.Emit(OpCodes.Ldsfld, fexpr.Spec);
                ec.Emit(OpCodes.Brtrue, l_initialized);
            }
            ...
            ec.EmitInt(20);
            ec.Emit(OpCodes.Box, ec.BuiltinTypes.Int);
            ec.Emit(OpCodes.Stsfld, fexpr.Spec);
        ...
        protected override void CloneTo(CloneContext clonectx, Statement t)
        {
            SynchronizedMethod target = (SynchronizedMethod)t;
    ...
    public class GroupAssign : Statement
    {
        ...
        public override bool Resolve(BlockContext ec)
        {
            ...
            foreach (Expression expr in Left)
                left.Add(expr.Resolve(ec));

            foreach (Expression expr in Right)
                right.Add(expr.Resolve(ec));
