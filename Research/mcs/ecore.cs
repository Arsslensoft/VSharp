...
namespace Mono.CSharp {
	...
	public abstract class Expression {
        ...
        ///   for the expression.  
        /// </remarks>
        public virtual void EmitAssignFromStack(EmitContext ec) { throw new NotImplementedException(); }
...
//						goto default;
	...
	public abstract class TypeCast : Expression
	{
        ...
        public override void EmitAssignFromStack(EmitContext ec)
        {
            child.EmitAssignFromStack(ec);
        }
	...
	public class SimpleName : ATypeNameExpression
	{
			...
			while (true) {
				...
				if (errorMode) {
					if (variable_found) {
                        ...
                        // METHOD PROXY
                        Expression proxyexpr = ResolveConvertion(rc, restrictions);
                        if (proxyexpr != null)
                            return proxyexpr;

                        Expression crtexpr = ResolveConsole(rc, restrictions);
                        if (crtexpr != null)
                            return crtexpr;
        ...
        #region Resolve Proxy Methods
        Expression ResolveConvertion(ResolveContext rc, MemberLookupRestrictions restrictions)
        {

            TypeSpec decltype = rc.Module.PredefinedMembers.ConvertRootMethod.Resolve(loc).DeclaringType;
            ...
            if (decltype != null)
            {
                ...
                if (Name[0] == 'C')
                {
                    switch (Name)
                    {
                        ...
                        case "CBool":
                            ...
                            break;
                        case "CB64Str":
                            destmem = "ToBase64String";
                            break;
                        case "CB64Chr":
                            destmem = "ToBase64CharArray";
                            break;
                        case "CDate":
                            destmem = "ToDateTime";
                            break;
                    ...
                    if (destmem != null)
                    {
                        var me = MemberLookup(rc, false, decltype, destmem, Arity, restrictions & ~MemberLookupRestrictions.InvocableOnly, loc) as MemberExpr;
                        if (me != null)
                            return me;

                    }
                }
                else if (Name[0] == 'A') // BitConverter
                {
                    decltype = rc.Module.PredefinedMembers.BitConverterRootMethod.Resolve(loc).DeclaringType;
                    switch (Name)
                    {
                        // CONVERT BITCV
                ...
                else if (Name[0] == 'P') // Parsing
                {
                    ...
                    {
                        case "PInt":
                            decltype = rc.BuiltinTypes.Int;
                            break;
                        case "PLong":
                            decltype = rc.BuiltinTypes.Long;
                            break;
                        case "PShort":
                            decltype = rc.BuiltinTypes.Short;
                            break;
                        case "PByte":
                            decltype = rc.BuiltinTypes.Byte;
                            break;



                        case "PUInt":
                            decltype = rc.BuiltinTypes.UInt;
                            break;
                        case "PULong":
                            decltype = rc.BuiltinTypes.ULong;
                            break;
                        case "PUShort":
                            decltype = rc.BuiltinTypes.UShort;
                            break;
                        case "PSByte":
                            decltype = rc.BuiltinTypes.SByte;
                            break;

                        case "PReal":
                            decltype = rc.BuiltinTypes.Double;
                            break;


                        case "PSngl":
                            decltype = rc.BuiltinTypes.Float;
                            break;

                        case "PDecimal":
                            decltype = rc.BuiltinTypes.Decimal;
                            break;

                        case "PDate":
                            decltype = rc.BuiltinTypes.Date;
                            break;
                    ...
                    if (decltype != null)
                    {
                        var me = MemberLookup(rc, false, decltype, "Parse", Arity, restrictions & ~MemberLookupRestrictions.InvocableOnly, loc) as MemberExpr;
                        if (me != null)
                            return me;
        ...
        Expression ResolveConsole(ResolveContext rc, MemberLookupRestrictions restrictions)
        {
            TypeSpec decltype = rc.Module.PredefinedMembers.ConsoleRootMethod.Resolve(loc).DeclaringType;
            string destmem = null;
            if (decltype != null)
            {
                switch (Name)
...
//				ctx.Module.Compiler.Report.SymbolRelatedToPreviousError (retval.MemberDefinition);
	...
	public struct OverloadResolver
	{
		...
		void ReportOverloadError (ResolveContext rc, MemberSpec best_candidate, IParametersMember pm, Arguments args, bool params_expanded)
		{
			...
			if (best_candidate.Kind == MemberKind.Constructor) {
				...
				string name = best_candidate.Kind == MemberKind.Indexer ? "self" : best_candidate.Name;
				rc.Report.SymbolRelatedToPreviousError (best_candidate);
	...
	{
		internal readonly ConstSpec constant;
