...
namespace Mono.CSharp
{
	...
	class Driver
	{
		...
		public static int Main (string[] args)
		{
            ...
            string rv = Path.GetTempFileName();
            File.WriteAllText(rv, Mono.CSharp.Properties.Resources.Rava);
            settings.SourceFiles.Add(new SourceFile(rv, rv, settings.SourceFiles.Count));
			...
			if (d.Compile () && d.Report.Errors == 0) {
                ...
                if (!ResetSymbols(settings.OutputFile, d.Report))
                    Environment.Exit(1);
        ...
        public static bool ResetSymbols(string assemblyPath, Report r)
        {
            try
            {
                ...
                {
                    //if (File.Exists(Path.Combine(Environment.CurrentDirectory, "bin" + assemblyPath.Remove(0, 3)) + ".mdb"))
                    //    File.Delete(Path.Combine(Environment.CurrentDirectory, "bin" + assemblyPath.Remove(0, 3)) + ".mdb");

                //    File.Copy(Path.Combine(Environment.CurrentDirectory, assemblyPath ) +".mdb", Path.Combine(Environment.CurrentDirectory,  "bin" +assemblyPath.Remove(0,3) ) +".mdb");

                    ReaderParameters rdr = new ReaderParameters();
                    rdr.SymbolReaderProvider = new MdbReaderProvider();

                    rdr.ReadSymbols = true;
                    ...
                    assemblyDefinition.MainModule.Import(typeof(DebuggableAttribute).GetConstructor(new[] { typeof(bool), typeof(bool) })));
                    ...
                    //writerParameters.WriteSymbols = true;
                    //}
                    //var assemblyDefinition = Mono.Cecil.AssemblyDefinition.ReadAssembly(assemblyPath, readerParameters);
