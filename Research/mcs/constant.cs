...
namespace Mono.CSharp {
    ...
    public class ComplexConstant : Constant
    {
        public readonly Complex Value;

        public ComplexConstant(BuiltinTypes types, Complex d, Location loc)
            : this(types.Complex, d, loc)
        {
        }

        public ComplexConstant(TypeSpec type, Complex d, Location loc)
            : base(loc)
        {
            this.type = type;
            eclass = ExprClass.Value;
        ...
        {
            MethodSpec m;
            ec.Emit(OpCodes.Ldc_R8, Value.Real);
            ec.Emit(OpCodes.Ldc_R8, Value.Imaginary);

            m = ec.Module.PredefinedMembers.ComplexCtor.Resolve(loc);
            if (m != null)
            {
        ...
        public override Constant ConvertExplicitly(bool in_checked_context, TypeSpec target_type)
        {
            ...
            {
                case BuiltinTypeSpec.Type.SByte:
                    return new SByteConstant(target_type, (sbyte)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Byte:
                    return new ByteConstant(target_type, (byte)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Short:
                    return new ShortConstant(target_type, (short)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.UShort:
                    return new UShortConstant(target_type, (ushort)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Int:
                    return new IntConstant(target_type, (int)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.UInt:
                    return new UIntConstant(target_type, (uint)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Long:
                    return new LongConstant(target_type, (long)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.ULong:
                    return new ULongConstant(target_type, (ulong)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Char:
                    return new CharConstant(target_type, (char)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Float:
                    return new FloatConstant(target_type, (float)Value.Magnitude, loc);
                case BuiltinTypeSpec.Type.Double:
                    return new DoubleConstant(target_type, Value.Magnitude, loc);
            }
        ...
        public override string GetValueAsLiteral()
        {
            ...
            return Value.Real.ToString() + "-" +Value.Imaginary.ToString()+ "J";
            else return Value.Real.ToString() + "+" + Value.Imaginary.ToString() + "J";
        }
    ...
    public class VLongConstant : Constant
    {
        public readonly VLong Value;

        public VLongConstant(BuiltinTypes types, VLong d, Location loc)
            : this(types.VLong, d, loc)
        {
        }

        public VLongConstant(TypeSpec type, VLong d, Location loc)
            : base(loc)
        {
            this.type = type;
            eclass = ExprClass.Value;
        ...
        public override void Emit(EmitContext ec)
        {
            ...
            ec.Emit(OpCodes.Ldstr, Value.ToString());
          

            m = ec.Module.PredefinedMembers.VLongParse.Resolve(loc);
            if (m != null)
            {
    ...
    public class QuadConstant : Constant
    {
        public readonly Quad Value;

        public QuadConstant(BuiltinTypes types, Quad d, Location loc)
            : this(types.Quad, d, loc)
        {
        }

        public QuadConstant(TypeSpec type, Quad d, Location loc)
            : base(loc)
        {
            this.type = type;
            eclass = ExprClass.Value;
        ...
        public override void Emit(EmitContext ec)
        {
            ...
            m = ec.Module.PredefinedMembers.QuadCtor.Resolve(loc);
            if (m != null)
            {
                ec.Emit(OpCodes.Call, m);
            }
        ...
        public override Constant ConvertExplicitly(bool in_checked_context, TypeSpec target_type)
        {
            switch (target_type.BuiltinType)
            {
                case BuiltinTypeSpec.Type.SByte:
                    return new SByteConstant(target_type, (sbyte)Value, loc);
                case BuiltinTypeSpec.Type.Byte:
                    return new ByteConstant(target_type, (byte)((ulong)Value), loc);
                case BuiltinTypeSpec.Type.Short:
                    return new ShortConstant(target_type, (short)Value, loc);
                case BuiltinTypeSpec.Type.UShort:
                    return new UShortConstant(target_type, (ushort)((ulong)Value), loc);
                case BuiltinTypeSpec.Type.Int:
                    return new IntConstant(target_type, (int)Value, loc);
                case BuiltinTypeSpec.Type.UInt:
                    return new UIntConstant(target_type, (uint)((ulong)Value), loc);
                case BuiltinTypeSpec.Type.Long:
                    return new LongConstant(target_type, (long)Value, loc);
                case BuiltinTypeSpec.Type.ULong:
                    return new ULongConstant(target_type, (ulong)Value, loc);
                case BuiltinTypeSpec.Type.Char:
                    return new CharConstant(target_type, (char)((ulong)Value), loc);
                case BuiltinTypeSpec.Type.Float:
                    return new FloatConstant(target_type, (float)Value, loc);
                case BuiltinTypeSpec.Type.Double:
                    return new DoubleConstant(target_type, (double)Value, loc);
    ...
    public class TimeConstant : Constant
    {
        public TimeConstant(BuiltinTypes types, TimeSpan s, Location loc)
            : this(types.Time, s, loc)
        {
        }

        public TimeConstant(TypeSpec type, TimeSpan s, Location loc)
            : base(loc)
        {
            this.type = type;
            eclass = ExprClass.Value;
        ...
        protected TimeConstant(Location loc)
            : base(loc)
        {
        }
        ...
        public override void Emit(EmitContext ec)
        {
            ...
            MethodSpec m;
            m = ec.Module.PredefinedMembers.TimeSpanCtor.Resolve(loc);
            if (m != null)
            {
                ec.Emit(OpCodes.Newobj, m);
            }
    ...
    {
        public DateConstant(BuiltinTypes types, DateTime s, Location loc)
            : this(types.Date, s, loc)
        {
        }

        public DateConstant(TypeSpec type, DateTime s, Location loc)
            : base(loc)
        {
            this.type = type;
            eclass = ExprClass.Value;
        ...
        protected DateConstant(Location loc)
            : base(loc)
        {
        }
        ...
        public override void Emit(EmitContext ec)
        {
            ec.ig.Emit(OpCodes.Ldc_I8, Value.ToBinary());
            MethodSpec m;
            m = ec.Module.PredefinedMembers.DateTimeParse.Resolve(loc);
            if (m != null)
            {
                ec.Emit(OpCodes.Call, m);
            }
