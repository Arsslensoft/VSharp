...
namespace Mono.CSharp
{
	...
	sealed class StaticImporter : MetadataImporter
	{
		...
		public void InitializeBuiltinTypes (BuiltinTypes builtin, Assembly corlib, Assembly ravalib)
		{
			//
			// Setup mapping for build-in types to avoid duplication of their definition
			//
			foreach (var type in builtin.AllTypes) {
                if(type.Namespace == "System.Rava")
				compiled_types.Add (ravalib.GetType (type.FullName), type);
                else
	...
	class StaticLoader : AssemblyReferencesLoader<Assembly>, IDisposable
	{
		...
		public override void LoadReferences (ModuleContainer module)
		{
			...
			base.LoadReferencesCore (module, out corlib,out ravalib, out loaded);
			...
			if (corlib == null) {
				...
				importer.InitializeBuiltinTypes (compiler.BuiltinTypes, corlib,ravalib);
				importer.ImportAssembly (corlib, module.GlobalRootNamespace);
                importer.ImportAssembly(ravalib, module.GlobalRootNamespace);
			}
