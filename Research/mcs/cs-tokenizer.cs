...
namespace Mono.CSharp
{
	...
	public class Tokenizer : yyParser.yyInput
	{
		...
		static Tokenizer ()
		{
            ...
            AddKeyword("super", Token.BASE);
			AddKeyword ("bool", Token.BOOL);
			AddKeyword ("leave", Token.BREAK);
			AddKeyword ("byte", Token.BYTE);
			AddKeyword ("case", Token.CASE);
			AddKeyword ("except", Token.CATCH);
			AddKeyword ("char", Token.CHAR);
			AddKeyword ("checked", Token.CHECKED);
			AddKeyword ("class", Token.CLASS);
			AddKeyword ("const", Token.CONST);
			AddKeyword ("persist", Token.CONTINUE);
			AddKeyword ("decimal", Token.DECIMAL);
            AddKeyword("complex", Token.COMPLEX);
            AddKeyword("pointer", Token.POINTER);
            AddKeyword("upointer", Token.UPOINTER);
            AddKeyword("vlong", Token.VLONG);
            AddKeyword("quad", Token.QUAD);
            AddKeyword("date", Token.DATE);
            AddKeyword("time", Token.TIME);
			...
			AddKeyword ("native", Token.EXTERN);
			...
			AddKeyword ("friend", Token.INTERNAL);
			AddKeyword ("is", Token.IS);
			AddKeyword ("lock", Token.LOCK);
            AddKeyword("assert", Token.ASSERT);
            AddKeyword("restrict", Token.RESTRICT);
            AddKeyword("asm", Token.ASM);
            AddKeyword("sync", Token.SYNC);
            AddKeyword("safe", Token.SAFE);
            AddKeyword("sym", Token.SYM);
			AddKeyword ("long", Token.LONG);
			AddKeyword ("package", Token.NAMESPACE);
            ...
            AddKeyword("import", Token.INCLUDE);
            AddKeyword("virt", Token.VIRTUAL);
			AddKeyword ("sub", Token.VOID);
			AddKeyword ("volatile", Token.VOLATILE);
			AddKeyword ("while", Token.WHILE);
            AddKeyword("loop", Token.LOOP);
			AddKeyword ("shared", Token.PARTIAL);
        ...
        static TypeCode real_type_suffix(int c, ref RavaPredefinedTypes rtc)
		{
            ...
            case 'Q':
            case 'q':
                rtc = RavaPredefinedTypes.Quad;
                return TypeCode.Object;
            case 'V':
            case 'v':
                rtc = RavaPredefinedTypes.VLong;
                return TypeCode.Object;

            case 'T':
            case 't':
                rtc = RavaPredefinedTypes.Time;
                return TypeCode.Object;
            case 'J':
            case 'j':
                rtc = RavaPredefinedTypes.Complex;
                return TypeCode.Object;
			default:
		...
		ILiteralConstant adjust_real (TypeCode t, RavaPredefinedTypes rpt,Location loc)
		{
			string s = new string (number_builder, 0, number_pos);
			const string error_details = "Floating-point constant is outside the range of type `{0}'";
            if (rpt != RavaPredefinedTypes.None && t == TypeCode.Object)
            {
                if (rpt == RavaPredefinedTypes.Complex)
                {
                    try
                    {
                        return new ComplexLiteral(context.BuiltinTypes,new Complex(0,  Double.Parse(s, styles, csharp_format_info)), loc);
                    }
                    catch (OverflowException)
                    {
                        Report.Error(594, Location, error_details, "complex");
                        return new ComplexLiteral(context.BuiltinTypes, new Complex(0, 0), loc);
                    }
                }
                else if (rpt == RavaPredefinedTypes.VLong)
                {
                    
                        try
                        {
                            return new VLongLiteral(context.BuiltinTypes, VLong.Parse(s, styles, csharp_format_info), loc);
                        }
                        catch (OverflowException)
                        {
                            Report.Error(594, Location, error_details, "vlong");
                            return new VLongLiteral(context.BuiltinTypes, new VLong( 0), loc);
                        }
                    
                }
                else if (rpt == RavaPredefinedTypes.Time)
                {

                    try
                    {
                        return new TimeLiteral(context.BuiltinTypes, new TimeSpan(long.Parse(s, styles, csharp_format_info)), loc);
                    }
                    catch (OverflowException)
                    {
                        Report.Error(594, Location, error_details, "time");
                        return new TimeLiteral(context.BuiltinTypes, new TimeSpan(0), loc);
                    }

                }
                else if (rpt == RavaPredefinedTypes.Quad)
                {

                    try
                    {
                        return new QuadLiteral(context.BuiltinTypes, Quad.Parse(s), loc);
                    }
                    catch (OverflowException)
                    {
                        Report.Error(594, Location, error_details, "quad");
                        return new QuadLiteral(context.BuiltinTypes, Quad.Zero, loc);
                    }
		...
		int is_number (int c, bool dotLead)
		{
            ...
            RavaPredefinedTypes rpt = RavaPredefinedTypes.None;
			var type = real_type_suffix (c,ref rpt);
        ...
        internal static List<AsmCodeDef> AsmCodes = new List<AsmCodeDef>();
        ...
        RavaPredefinedTypes ParseStringLiteral(int c)
        {
            ...
            switch (c)
            {
                case 'd':
                case 'D':
                    return RavaPredefinedTypes.Date;
                case 't':
                case 'T':
                    return RavaPredefinedTypes.Time;
            }
            return RavaPredefinedTypes.None;
        }
		int consume_string (bool quoted)
		{
			...
			while (true){
				...
				if (c == '"') {
                    ...
                    RavaPredefinedTypes rpt = ParseStringLiteral(peek_char());
                    if (rpt == RavaPredefinedTypes.None)
                    {
                        ILiteralConstant res = new StringLiteral(context.BuiltinTypes, CreateStringFromBuilder(pos), start_location);
                        val = res;
                    }
                    else if(rpt == RavaPredefinedTypes.Date)
                    {
                        get_char();
                        ILiteralConstant res = new DateLiteral(context.BuiltinTypes,DateTime.Parse( CreateStringFromBuilder(pos)), start_location);
                        val = res;
                    }
#if FULL_AST
				...
				if (c == '\n' || c == UnicodeLS || c == UnicodePS) {
					if (!quoted) {
                        ...
                        {
                            get_char();
                            val = new DateLiteral(context.BuiltinTypes, DateTime.Parse(CreateStringFromBuilder(pos)), start_location);
