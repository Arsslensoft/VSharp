...
public class TryFinally : TryFinallyBlock
	{
		...
		protected override void EmitTryBody (EmitContext ec)
		{
			if (fini.HasAwait) {
				...
				ec.TryFinallyUnwind.Addition (this);
				stmt.Emit (ec);
		...
		void EmitRedirectedExit (EmitContext ec, Label label, AsyncInitializer initializer, bool setReturnState)
		{
			if (redirected_jumps == null) {
				...
				// Addition fallthrough label
				redirected_jumps.Addition (ec.DefineLabel ());
			...
			if (index < 0) {
				redirected_jumps.Addition (label);
